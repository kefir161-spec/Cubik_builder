...
// ===== App init after models loaded =====
  ...
  if(selectedBlock && selectedList().length>0){
    ...
    try{
      ...
      console.log('[Init] __INITIAL_PROJECT__:', initial ? 'exists' : 'null');
      if(initial && typeof importProjectJSONFromText === 'function'){
        ...
        console.log('[Init] loadedInitial after import:', loadedInitial);
      }
    }catch(e){
    ...
    // Check if autosave data exists (will be restored after window.load)
    ...
    try{
      ...
      if(autosaveData){
        ...
        console.log('[Init] Autosave found:', arr.length, 'objects');
      } else {
        console.log('[Init] No autosave data');
      }
    }catch(e){
      console.log('[Init] Autosave check error:', e);
    }
    ...
    if(!loadedInitial && !hasAutosave){
      console.log('[Init] Creating default cube (no autosave, no initial)');
      ...
      console.log('[Init] Skipping default cube (hasAutosave=' + hasAutosave + ', loadedInitial=' + loadedInitial + ')');
    }
animate();
    updateCounter();
    selectGallery('Bion');
...
// initial undo snapshot
    console.log('[Init] Final state: objects.length =', objects.length);
    pushState();
  ...
  for(var si=0; si<snapArr.length; si++){
    ...
    if(existingObj){
      ...
      if(s.type === 'solid'){
        ...
        for(var dir in s.faces){
          ...
          if(faceMesh){
            ...
            // If desiredType is FromEdited or unavailable, use existing kind or Bion
            if (!faceGeoms[desiredType]) {
              desiredType = existingObj.userData.kind;
              if (!faceGeoms[desiredType]) desiredType = 'Bion';
            }
            try{
      ...
      if(s.type==='solid'){
        ...
        // Determine base kind - if kind is FromEdited or unavailable, use first face's type
        var baseKind = s.kind;
        if (!faceGeoms[baseKind]) {
          if (s.faces) {
            for (var fd in s.faces) {
              if (s.faces.hasOwnProperty(fd) && s.faces[fd].faceType) {
                var ft = s.faces[fd].faceType;
                if (faceGeoms[ft]) {
                  baseKind = ft;
                  break;
                }
          ...
          if (!faceGeoms[baseKind]) baseKind = 'Bion';
        }
        
        var grp = buildCubeGroup(baseKind, '#7D7F7D');
        grp.uuid = s.uuid; // сохраняем оригинальный UUID
        grp.userData.kind = s.kind; // Keep original kind
        ...
        for(var dir in s.faces){
          ...
          if(faceMesh){
            // При необходимости подменяем геометрию грани в соответствии с faceType из снапшота
            var desiredType = fs.faceType || baseKind;
            if (!faceGeoms[desiredType]) desiredType = baseKind;
            try{
              if (desiredType && faceGeoms && faceGeoms[desiredType] && faceGeoms[desiredType][dir]){
                var newGeom = faceGeoms[desiredType][dir];
                var aligned = alignGeomPlaneTo(faceMesh.geometry, newGeom, dir);
...
function loadSceneFromSnapshot(snapArr){
  ...
  console.log('[loadSceneFromSnapshot] Loading', snapArr.length, 'objects, current objects:', objects.length);
  console.trace('[loadSceneFromSnapshot] Called from:');
  ...
  console.log('[loadSceneFromSnapshot] Removing', toRemove.length, 'existing objects');
  ...
  for (var si = 0; si < snapArr.length; si++){
    ...
    if (s.type === 'solid'){
      ...
      var baseKind = s.kind;
      if (!faceGeoms[baseKind]) {
        // Try to get base kind from first face
        if (s.faces) {
          for (var fd in s.faces) {
            if (s.faces.hasOwnProperty(fd) && s.faces[fd].faceType) {
              var ft = s.faces[fd].faceType;
              // Use face type if it's a built-in kind
              if (faceGeoms[ft]) {
                baseKind = ft;
                break;
              }
        ...
        // Fallback to Bion if still not found
        if (!faceGeoms[baseKind]) {
          baseKind = 'Bion';
        }
        console.log('[loadSceneFromSnapshot] Replaced kind', s.kind, '->', baseKind);
      }
      
      var grp = buildCubeGroup(baseKind, '#7D7F7D');
      if (s.uuid) grp.uuid = s.uuid;
      // Keep original kind in userData for reference
      grp.userData.kind = s.kind;
      ...
      if (s.faces && grp.userData && grp.userData.faces){
        for (var dir in s.faces){
          ...
          var desiredType = fs.faceType || baseKind;
          // If desiredType is FromEdited or unavailable, use baseKind
          if (!faceGeoms[desiredType]) {
            desiredType = baseKind;
          }
...
// ===== Custom kinds serialization for JSON export/import =====
  ...
  // Built-in kinds (loaded from .obj, no need to serialize geometry)
  // Also includes 'FromEdited' and 'Kind-XXX' which are dynamically created from built-in types
  var BUILTIN_KINDS = ['box', 'Void', 'Zen', 'Zen/2', 'Bion'];
  
  function isBuiltinKind(kind) {
    if (!kind) return false;
    if (BUILTIN_KINDS.indexOf(kind) !== -1) return true;
    // FromEdited types are created from built-in types, no need to serialize
    if (kind.indexOf('FromEdited') === 0) return true;
    // Kind-XXX are created when copying complex blocks, also derived from built-in
    if (kind.indexOf('Kind-') === 0) return true;
    return false;
  }
  ...
  function serializeCustomKindsForSnapshot(snap){
    ...
    // Debug: log what kinds are used and which are built-in
    console.log('[Export] Used kinds:', Object.keys(usedKinds));
    console.log('[Export] CustomKinds keys:', Object.keys(customKinds));

    for(var kind in usedKinds){
      if(!Object.prototype.hasOwnProperty.call(usedKinds, kind)) continue;
      
      // Skip built-in kinds - they don't need geometry saved
      if(isBuiltinKind(kind)){
        console.log('[Export] Skipping built-in kind:', kind);
        continue;
      }
      
      var ck = customKinds[kind];
      if(!ck){
        console.log('[Export] Kind not in customKinds:', kind);
        continue;
      }

      console.log('[Export] Serializing custom kind:', kind);
    ...
    console.log('[Export] Result customKinds:', Object.keys(result));
  ...
  function restoreCustomKindsFromPayload(map){
    ...
    for(var kind in map){
      ...
      // Skip built-in kinds (already loaded from .obj)
...
// ===== JSON Export for backend sample =====
  function exportProjectJSON(){
    try{
      ...
      // Сохраняем кастомные типы, если есть (только не-встроенные)
      ...
      try{
        if(window.console && console.log){
          console.log('[Export] ' + sizeKB + ' KB, ' + snap.length + ' objects');
        }
      }catch(_){}
      ...
      if(window.CubikUI && CubikUI.showStatus){
        CubikUI.showStatus('Exported (' + sizeKB + ' KB)', true);
      }
    }catch(e){
