  ...
  function onMove(e){
    ...
    if(hit.object===ground){
      ...
      if (blk.userData && blk.userData.faceTypes) {
        ...
        var blkH = getHalf(baseKind);
        var localHit = new THREE.Vector3().copy(hit.point).sub(blk.position);
        ...
        if (outsideX || outsideY || outsideZ) {
          // Hit point is outside cube - determine which Flora face
          var maxDist = 0;
          var faces = [
            { dir: 'right',  dist: localHit.x - blkH.x, hasFlora: blk.userData.faceTypes.right === 'Flora' },
            { dir: 'left',   dist: -localHit.x - blkH.x, hasFlora: blk.userData.faceTypes.left === 'Flora' },
            { dir: 'top',    dist: localHit.y - blkH.y, hasFlora: blk.userData.faceTypes.top === 'Flora' },
            { dir: 'bottom', dist: -localHit.y - blkH.y, hasFlora: blk.userData.faceTypes.bottom === 'Flora' },
            { dir: 'front',  dist: localHit.z - blkH.z, hasFlora: blk.userData.faceTypes.front === 'Flora' },
            { dir: 'back',   dist: -localHit.z - blkH.z, hasFlora: blk.userData.faceTypes.back === 'Flora' }
          ];
      ...
      // For Flora hit, override axisN based on flora face direction
      if (isFloraHit && floraFaceDir) {
        if (floraFaceDir === 'right' || floraFaceDir === 'left') {
          axisN = 'x'; axisU = 'y'; axisV = 'z';
          n.set(floraFaceDir === 'right' ? 1 : -1, 0, 0);
        } else if (floraFaceDir === 'top' || floraFaceDir === 'bottom') {
          axisN = 'y'; axisU = 'x'; axisV = 'z';
          n.set(0, floraFaceDir === 'top' ? 1 : -1, 0);
        } else if (floraFaceDir === 'front' || floraFaceDir === 'back') {
          axisN = 'z'; axisU = 'x'; axisV = 'y';
          n.set(0, 0, floraFaceDir === 'front' ? 1 : -1);
        }
      }
      ...
      // If snapping to Flora face, add offset to place cube outside the bowl
      // EXCEPT if ghost also has Flora on the touching face (Flora-to-Flora can overlap)
      if (isFloraHit) {
        // Determine which face of ghost will touch the target
        var ghostTouchFace = null;
        if (floraFaceDir === 'front') ghostTouchFace = 'back';
        else if (floraFaceDir === 'back') ghostTouchFace = 'front';
        else if (floraFaceDir === 'right') ghostTouchFace = 'left';
        else if (floraFaceDir === 'left') ghostTouchFace = 'right';
        
        // Check if ghost has Flora on that face
        ...
        // Only add offset if ghost doesn't have Flora on touching face
        if (!ghostHasFlora) {
          var floraOffset = typeof FLORA_BOWL_PROTRUSION !== 'undefined' ? FLORA_BOWL_PROTRUSION : 0.46;
          floraOffset += 0.05; // Add small gap to prevent touching
          if (n[axisN] > 0) {
            pos[axisN] += floraOffset;
          } else {
            pos[axisN] -= floraOffset;
          }
        }
      ...
      if (!isFloraHit) {
        ...
        if (ghostHasFloraOnTouch) {
          ...
          floraOffsetGhost += 0.02; // Small gap
          if (n[axisN] > 0) {
      ...
      if (isFloraHit && axisN !== 'y') {
        // Center on target block (same Y and same position on the other horizontal axis)
      ...
      if (axisN === 'y' && blk && blk.userData && blk.userData.faceTypes) {
...
for(var i=0;i<dirs.length;i++){
  ...
  function replaceFaces(){
    ...
    // Flora rule: only side faces allowed (not top/bottom)
    if(selectedFaceType === 'Flora'){
      var topBottom = { top:1, bottom:1 };
      for(var ff=0; ff<dirs.length; ff++){
        var fDir = String(dirs[ff]).toLowerCase();
        if(topBottom[fDir]){
          msg('Flora can only be applied to side faces (not top/bottom)', false);
          return false;
        }
      }
    }
    ...
    Promise.all(promises).then(function(results){
      ...
      try {
        if (baseGeom['Flora']) {
          ...
          faceGeoms['Flora'] = {
            top: null,    // Flora not allowed on top
            bottom: null, // Flora not allowed on bottom
          ...
          ['front','back','left','right'].forEach(function(d){
      ...
      if(s.type === 'solid'){
        ...
        for(var dir in s.faces){
          ...
          if(faceMesh){
            ...
            // Only replace geometry if face type actually changed
      ...
      if(s.type==='solid'){
        ...
        for(var dir in s.faces){
          ...
          if(faceMesh){
            ...
            // Only replace geometry if face type differs from cube type
            var needsGeometryReplacement = (desiredType !== s.kind);
    ...
    if (s.type === 'solid'){
      ...
      if (s.faces && grp.userData && grp.userData.faces){
        for (var dir in s.faces){
          ...
          // Only replace geometry if face type differs from cube type
          // If same type, buildCubeGroup already created correct geometry
          var needsGeometryReplacement = (desiredType !== s.kind);
          
          if (needsGeometryReplacement) {
            // Check if target faceType geometry exists
            if (!faceGeoms || !faceGeoms[desiredType]) {
              console && console.warn && console.warn('[loadSceneFromSnapshot] faceGeoms missing for type:', desiredType);
            } else if (!faceGeoms[desiredType][dir]) {
              console && console.warn && console.warn('[loadSceneFromSnapshot] faceGeoms[' + desiredType + '][' + dir + '] is missing');
            }
  ...
  function restoreCustomKindsFromPayload(map){
    ...
    for(var kind in map){
      ...
      for(var i = 0; i < dirs.length; i++){
        ...
        if(faceGeoms && faceGeoms[faceType] && faceGeoms[faceType][d]){
          faceMap[d] = faceGeoms[faceType][d];
        } else if(faceGeoms && faceGeoms[baseKind] && faceGeoms[baseKind][d]){
          // Fallback to baseKind geometry
          faceMap[d] = faceGeoms[baseKind][d];
        } else if(faceGeoms && faceGeoms['Bion'] && faceGeoms['Bion'][d]){
        ...
        } else if(faceGeoms && faceGeoms['box'] && faceGeoms['box'][d]){
