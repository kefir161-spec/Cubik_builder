...
// Zen/2 orientation state for preview + ghost
  ...
  function faceMetaFromBlock(blk){
    ...
    for(var i=0;i<dirs.length;i++){
      ...
      // Определяем тип грани с fallback для Zen/2
      var faceType = (blk.userData.faceTypes && blk.userData.faceTypes[dir]) || null;
      if (!faceType) {
        if (blk.userData.kind === 'Zen/2' && (dir === 'top' || dir === 'bottom')) {
          faceType = 'Bion';
        } else {
      ...
      // IMPORTANT: do NOT bake Flora upright roll correction into saved geometry.
// Flora face meshes may be dynamically rolled at runtime (applyFloraUprightRoll) to keep bowls "up" in world space.
// When saving, we must use the *base* quaternion (captured before roll) so reload does not double-apply the correction.
      var qSave = null;
      try{
        if(faceType === 'Flora' && f.userData && f.userData._floraBaseQuat){
          qSave = f.userData._floraBaseQuat.clone();
        }
      }catch(_){}
      if(!qSave){
        // Prefer quaternion (more reliable than Euler if mesh was manipulated)
        qSave = (f.quaternion) ? f.quaternion.clone() : new THREE.Quaternion().setFromEuler(f.rotation.clone());
      }
      ...
      // Preserve Flora pre-positioning marker so future alignment does not shift it.
      if(faceType === 'Flora'){
        g.userData = g.userData || {};
        g.userData.prePositioned = true;
        g.userData.floraFace = dir;
      }
  ...
  function registerCustomKindFromBlock(blk, name){
    ...
    // and remember its baseKind (used for robust save/load fallback).
    var zen2Like = false;
    var baseKind = 'Bion';
    try{
      if (blk && blk.userData){
        var srcKind = blk.userData.kind;
        if (srcKind === 'Zen_2') srcKind = 'Zen/2';
        if (srcKind){
          if (srcKind === 'Zen/2'){
            zen2Like = true;
            baseKind = 'Zen/2';
          } else if (customKinds && customKinds[srcKind]) {
            var srcCk = customKinds[srcKind];
            if (srcCk.zen2Like) zen2Like = true;
            if (srcCk.baseKind) baseKind = srcCk.baseKind;
            else baseKind = srcKind;
          } else {
            baseKind = srcKind;
          }
        }
  ...
  function buildGroupFromCustomKind(kind){
    ...
    for(var i=0;i<dirs.length;i++){
      ...
      var ft = (data.faceTypes && data.faceTypes[dir]) ? data.faceTypes[dir] : null;
      if(!ft){
        var bk = data.baseKind || kind;
        if ((data.zen2Like || bk === 'Zen/2') && (dir === 'top' || dir === 'bottom')) ft = 'Bion';
        else if (data.zen2Like || bk === 'Zen/2') ft = 'Zen/2';
        else ft = bk;
      }
      group.userData.faceTypes[dir] = ft;
      pickables.push(m);
    }
    return group;
  }
...
// ===== Materials / block constructors =====
  ...
  function buildCubeGroup(type,colorHex){
    ...
    for(var i=0;i<dirs.length;i++){
      ...
      // Zen/2 special rule: top/bottom are Bion, other 4 faces are Zen/2
      if (type === 'Zen/2') {
      ...
      // Важно: если faceTypes не задан для конкретной грани, НЕЛЬЗЯ фолбечить на имя кастомного kind —
      // это приводило к "перепутыванию" типов при save/load. Фолбечим на baseKind и учитываем zen2Like.
      try{
        if (customKinds && customKinds[type]){
          var ck = customKinds[type];
          if (ck.faceTypes && ck.faceTypes[dir]) {
            faceTypeForDir = ck.faceTypes[dir];
          } else {
            var bk = ck.baseKind || type;
            if ((ck.zen2Like || bk === 'Zen/2') && (dir === 'top' || dir === 'bottom')) {
              faceTypeForDir = 'Bion';
            } else if (ck.zen2Like || bk === 'Zen/2') {
              faceTypeForDir = 'Zen/2';
            } else {
              faceTypeForDir = bk;
            }
    ...
    for(var i=0;i<objects.length;i++){
      ...
      if(o.userData.solid){
        ...
        for(var j=0;j<dirs.length;j++){
          ...
          // Fallback для Zen/2: top/bottom → Bion
          if (!t) {
            if (o.userData.kind === 'Zen/2' && (d === 'top' || d === 'bottom')) {
              t = 'Bion';
            } else {
  ...
  for(var i=0;i<objects.length;i++){
    ...
    if(o.userData.faces){
      ...
      for(var dir in o.userData.faces){
        ...
        // Fallback для Zen/2: top/bottom → Bion
        if (!fType) {
          if (o.userData.kind === 'Zen/2' && (dir === 'top' || dir === 'bottom')) {
            fType = 'Bion';
          } else {
      ...
      if(s.type === 'solid'){
        ...
        for(var dir in s.faces){
          ...
          if(faceMesh){
            ...
            var desiredType = fs.faceType;
            
            // Fallback для обратной совместимости со старыми JSON
            if (!desiredType) {
              if (s.kind === 'Zen/2' && (dir === 'top' || dir === 'bottom')) {
                desiredType = 'Bion';
              } else {
                desiredType = s.kind;
              }
            ...
            var currentType = existingObj.userData.faceTypes && existingObj.userData.faceTypes[dir];
            
            // Заменяем геометрию только если:
            // 1. Тип изменился (desiredType !== currentType)
            // 2. Для Zen/2: разрешаем только Flora
            // 3. Это НЕ кастомный kind (кроме Flora)
            var isZen2Like = (s.kind === 'Zen/2') || (customKinds && customKinds[s.kind] && customKinds[s.kind].zen2Like);
            var isCustom = customKinds && customKinds[s.kind];
            var isFloraOnZen2 = isZen2Like && desiredType === 'Flora';
            var needsGeometryReplacement = (desiredType !== currentType) && (!isZen2Like || isFloraOnZen2) && !isCustom;
            
            if (needsGeometryReplacement) {
              try{
                if (desiredType && faceGeoms && faceGeoms[desiredType] && faceGeoms[desiredType][dir]){
                  ...
                  // Для Flora на Zen/2: корректируем позицию
                  if (isFloraOnZen2) {
                    var oldGeomClone = faceMesh.geometry.clone();
                    oldGeomClone.computeBoundingBox();
                    var oldBB = oldGeomClone.boundingBox;
                    var axisInfo = axisForFace(dir);
                    var axis = axisInfo.axis;
            ...
            if (fs.faceRotation && Array.isArray(fs.faceRotation)) {
              faceMesh.rotation.set(fs.faceRotation[0], fs.faceRotation[1], fs.faceRotation[2]);
            } else if (isFloraOnZen2 && needsGeometryReplacement) {
              try {
                var faceNormalVectors = {
                ...
                var localNormal = faceNormalVectors[dir];
                var floraLocalUp = floraUpVectors[dir];
                if (localNormal && floraLocalUp) {
                  var worldNormal = localNormal.clone().applyQuaternion(existingObj.quaternion);
                  var worldUp = new THREE.Vector3(0, 1, 0);
                  var dotWU = worldUp.dot(worldNormal);
                  var desiredWorldUp = worldUp.clone().sub(worldNormal.clone().multiplyScalar(dotWU));
                  
                  if (desiredWorldUp.lengthSq() > 0.0001) {
                    desiredWorldUp.normalize();
                    var invCubeQ = existingObj.quaternion.clone().invert();
                    var desiredLocalUp = desiredWorldUp.clone().applyQuaternion(invCubeQ);
                    
                    var currentLocalUp = floraLocalUp.clone();
                    var dotCU = currentLocalUp.dot(localNormal);
                    var currentProjected = currentLocalUp.clone().sub(localNormal.clone().multiplyScalar(dotCU));
                    
                    var dotDU = desiredLocalUp.dot(localNormal);
                    var desiredProjected = desiredLocalUp.clone().sub(localNormal.clone().multiplyScalar(dotDU));
                    
                    if (currentProjected.lengthSq() > 0.0001 && desiredProjected.lengthSq() > 0.0001) {
                      currentProjected.normalize();
                      desiredProjected.normalize();
                      var cosAngle = currentProjected.dot(desiredProjected);
                      var sinAngle = new THREE.Vector3().crossVectors(currentProjected, desiredProjected).dot(localNormal);
      ...
      if(s.type==='solid'){
        ...
        for(var dir in s.faces){
          ...
          if(faceMesh){
            ...
            // Заменяем геометрию только если:
            // 1. desiredType отличается от kind куба
            // 2. Для Zen/2: разрешаем только Flora
            // 3. Это НЕ кастомный kind (кроме Flora)
            var isZen2Like = (s.kind === 'Zen/2') || (customKinds && customKinds[s.kind] && customKinds[s.kind].zen2Like);
            var isCustomKind = customKinds && customKinds[s.kind];
            var isFloraOnZen2 = isZen2Like && desiredType === 'Flora';
            var needsGeometryReplacement = (desiredType !== s.kind) && (!isZen2Like || isFloraOnZen2) && !isCustomKind;
            ...
            if (fs.faceRotation && Array.isArray(fs.faceRotation)) {
              ...
              try {
                ...
                var floraLocalUp = floraUpVectors[dir];
                if (localNormal && floraLocalUp) {
                  var worldNormal = localNormal.clone().applyQuaternion(grp.quaternion);
                  var worldUp = new THREE.Vector3(0, 1, 0);
                  var dotWU = worldUp.dot(worldNormal);
                  var desiredWorldUp = worldUp.clone().sub(worldNormal.clone().multiplyScalar(dotWU));
                  ...
                  if (desiredWorldUp.lengthSq() > 0.0001) {
                    desiredWorldUp.normalize();
                    var invCubeQ = grp.quaternion.clone().invert();
                    var desiredLocalUp = desiredWorldUp.clone().applyQuaternion(invCubeQ);
...
function loadSceneFromSnapshot(snapArr){
  ...
  console && console.log && console.log('[loadSceneFromSnapshot] Loading', snapArr.length, 'objects...');

  // Defensive helpers: tolerate legacy/partial snapshots without aborting the whole load
  function _arr3(a, d0, d1, d2){
    return (Array.isArray(a) && a.length >= 3) ? a : [d0, d1, d2];
  }

  // Полностью очищаем текущую сцену от блоков
  // Сначала собираем все объекты для удаления (чтобы избежать проблем с итерацией)
  var toRemove = objects.slice(); // копия массива
  
  for (var i = 0; i < toRemove.length; i++){
    var obj = toRemove[i];
    try{ removeWrapperForBlock(obj); }catch(e){ console.warn('[loadSceneFromSnapshot] removeWrapperForBlock error:', e); }
    try{ disposeObjectRecursive(obj); }catch(e){ console.warn('[loadSceneFromSnapshot] disposeObjectRecursive error:', e); }
    try{ 
      scene.remove(obj);
    }catch(e){ 
  ...
  for (var si = 0; si < snapArr.length; si++){
    ...
    if (s.type === 'solid'){
      ...
      var pS = _arr3(s.position, 0, 0, 0);
      var rS = _arr3(s.rotation, 0, 0, 0);
      var scS = _arr3(s.scale, 1, 1, 1);
      m.position.set(pS[0], pS[1], pS[2]);
      m.rotation.set(rS[0], rS[1], rS[2]);
      m.scale.set(scS[0], scS[1], scS[2]);
      ...
      var pG = _arr3(s.position, 0, 0, 0);
      var rG = _arr3(s.rotation, 0, 0, 0);
      var scG = _arr3(s.scale, 1, 1, 1);
      grp.position.set(pG[0], pG[1], pG[2]);
      grp.rotation.set(rG[0], rG[1], rG[2]);
      grp.scale.set(scG[0], scG[1], scG[2]);

      // Check if group has faces (fallback might have created them)
      var hasFaces = grp.userData && grp.userData.faces && Object.keys(grp.userData.faces).length > 0;
      if (!hasFaces) {
        console && console.warn && console.warn('[loadSceneFromSnapshot] Group has no faces for kind:', s.kind);
        skippedCount++;
      }
      ...
      if (s.faces && grp.userData && grp.userData.faces){
        for (var dir in s.faces){
          ...
          var faceMesh = grp.userData.faces[dir];
          if (!faceMesh) {
            console && console.warn && console.warn('[loadSceneFromSnapshot] Missing face mesh for', dir, 'in kind', s.kind);
            continue;
          }
          ...
          // Определяем нужно ли заменять геометрию:
          // - Для Zen/2: разрешаем замену ТОЛЬКО для Flora (специальная обработка)
          // - Для кастомных kinds с zen2Like: разрешаем Flora
          // - Для обычных kinds: заменять если faceType отличается
          var isZen2Like = (s.kind === 'Zen/2') || (customKinds && customKinds[s.kind] && customKinds[s.kind].zen2Like);
          var isCustom = customKinds && customKinds[s.kind];
          var isFloraOnZen2 = isZen2Like && desiredType === 'Flora';
          var needsGeometryReplacement = (desiredType !== s.kind) && (!isZen2Like || isFloraOnZen2) && !isCustom;
          ...
          if (needsGeometryReplacement) {
            try{
              if (desiredType && faceGeoms && faceGeoms[desiredType] && faceGeoms[desiredType][dir]){
                ...
                // Для Flora на Zen/2: корректируем позицию как в replaceFaces
                if (isFloraOnZen2) {
                  // Получаем bounding box текущей грани Zen/2 (реальный размер)
                  var oldGeomClone = faceMesh.geometry.clone();
                  oldGeomClone.computeBoundingBox();
                  var oldBB = oldGeomClone.boundingBox;
              ...
              console && console.warn && console.warn('[loadSceneFromSnapshot] Geometry replacement failed for', dir, eGeom);
            }
          }
          ...
          // Восстанавливаем поворот грани (важно для Flora на Zen/2)
          if (fs.faceRotation && Array.isArray(fs.faceRotation)) {
            faceMesh.rotation.set(fs.faceRotation[0], fs.faceRotation[1], fs.faceRotation[2]);
          } else if (isFloraOnZen2 && needsGeometryReplacement) {
            // Для старых JSON без faceRotation: вычисляем поворот Flora вокруг нормали грани
            try {
              var faceNormalVectors = {
                'left': new THREE.Vector3(-1, 0, 0),
                'right': new THREE.Vector3(1, 0, 0),
              ...
              console && console.warn && console.warn('[loadSceneFromSnapshot] Flora rotation compensation failed for', dir, eRot);
            }
          }
      ...
      if (hasFaces) loadedCount++;
    }
  }

  console && console.log && console.log('[loadSceneFromSnapshot] Loaded:', loadedCount, 'Skipped/Empty:', skippedCount);
...
// ===== Custom kinds serialization for JSON export/import =====
  ...
  function serializeCustomKindsForSnapshot(snap){
    ...
    for(var kind in usedKinds){
      ...
      // Lossless: сохраняем и метаданные, и геометрию кастомного вида.
      // restoreCustomKindsFromPayload уже умеет читать legacy-поля faceGeoms/mergedGeom.
      var out = {
        faceColors: ck.faceColors || {},
        faceTypes: ck.faceTypes || {},
        zen2Like: !!ck.zen2Like,
        baseKind: ck.baseKind || (ck.zen2Like ? 'Zen/2' : 'Bion')
      };

      // Serialize per-face geometries (legacy-compatible field name: faceGeoms)
      try{
        if(ck.faceGeoms){
          var fgOut = {};
          var hasAny = false;
          for(var i=0;i<dirs.length;i++){
            var d = dirs[i];
            var g = ck.faceGeoms[d];
            if(!g || typeof g.toJSON !== 'function') continue;
            try{
              var g2 = (g.clone && g.clone()) ? g.clone() : g;
              if(g2 && g2.attributes && g2.attributes.uv) g2.deleteAttribute('uv');
              fgOut[d] = serializeGeometryForJSON(g2);
              hasAny = true;
            }catch(eFace){
              try{
                fgOut[d] = serializeGeometryForJSON(g);
                hasAny = true;
              }catch(_){}
            }
          }
          if(hasAny) out.faceGeoms = fgOut;
        }
      }catch(e){}

      // Serialize merged geometry (legacy-compatible field name: mergedGeom)
      try{
        var mg = ck.mergedGeom || (baseGeom && baseGeom[kind]) || null;
        if(mg && typeof mg.toJSON === 'function'){
          var mg2 = (mg.clone && mg.clone()) ? mg.clone() : mg;
          if(mg2 && mg2.attributes && mg2.attributes.uv) mg2.deleteAttribute('uv');
          out.mergedGeom = serializeGeometryForJSON(mg2);
        }
      }catch(e){}
  ...
  function restoreCustomKindsFromPayload(map){
    ...
    for(var kind in map){
      ...
      if(customKinds && customKinds[kind]) continue;
      ...
      if(data.faceGeoms){
        for(var j = 0; j < dirs.length; j++){
          ...
          if(g){
            // If this face is Flora, restore the pre-positioned marker.
            // Deserialized BufferGeometry may lose userData; without this, alignGeomPlaneTo may shift Flora bowls.
            try{
              var ft = (data.faceTypes && data.faceTypes[dd]) || (faceTypes && faceTypes[dd]) || null;
              if(ft === 'Flora'){
                g.userData = g.userData || {};
                g.userData.prePositioned = true;
                g.userData.floraFace = dd;
              }
            }catch(_){}
            faceMap[dd] = g;
          }
