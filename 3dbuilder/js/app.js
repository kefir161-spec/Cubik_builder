      ...
      if(s.type === 'solid'){
        ...
        for(var dir in s.faces){
          ...
          if(faceMesh){
            ...
            // Проверяем доступна ли геометрия для желаемого типа
            var desiredGeomAvailable = faceGeoms && faceGeoms[desiredType] && faceGeoms[desiredType][dir];
            
            // Замена нужна если желаемый тип отличается от текущего И геометрия доступна
            var needsGeometryReplacement = (desiredType !== currentType) && desiredGeomAvailable;
            ...
            // ВСЕГДА сохраняем желаемый тип из снапшота
            ...
            // Для Flora: ВСЕГДА пересчитываем через applyFloraUprightRoll
            if (desiredType === 'Flora') {
              try {
                faceMesh.userData = faceMesh.userData || {};
                faceMesh.userData._floraBaseQuat = new THREE.Quaternion();
                applyFloraUprightRoll(faceMesh, existingObj, dir);
              } catch(eFlora) {
      ...
      if(s.type==='solid'){
        ...
        for(var dir in s.faces){
          ...
          if(faceMesh){
            ...
            // Определяем текущий тип грани созданного куба
            ...
            // Замена нужна если желаемый тип отличается от текущего И геометрия доступна
            var needsGeometryReplacement = (desiredType !== currentFaceType) && desiredGeomAvailable;
    ...
    if (s.type === 'solid'){
      ...
      if (s.faces && grp.userData && grp.userData.faces){
        for (var dir in s.faces){
          ...
          // Замена нужна если:
          // 1. Желаемый тип отличается от текущего
          // 2. Геометрия для желаемого типа доступна
          var needsGeometryReplacement = (desiredType !== currentFaceType) && desiredGeomAvailable;
          ...
          // ВСЕГДА сохраняем желаемый тип из JSON (даже если замена не произошла)
          // Это важно для правильной работы при повторном сохранении
          grp.userData.faceTypes[dir] = desiredType;
          ...
          // Для Flora: ВСЕГДА пересчитываем quaternion через applyFloraUprightRoll
          // чтобы гарантировать правильную ориентацию "чашей вверх"
          var finalFaceType = grp.userData.faceTypes[dir];
          if (finalFaceType === 'Flora') {
            try {
              // Инициализируем _floraBaseQuat как identity (базовая ориентация)
              faceMesh.userData = faceMesh.userData || {};
              faceMesh.userData._floraBaseQuat = new THREE.Quaternion();
              // Вычисляем правильный quaternion для Flora
              applyFloraUprightRoll(faceMesh, grp, dir);
            } catch(eFlora) {
              console && console.warn && console.warn('[loadSceneFromSnapshot] Flora orientation failed for', dir, eFlora);
              // Fallback: пробуем восстановить из JSON
              if (fs.faceQuaternion && Array.isArray(fs.faceQuaternion) && fs.faceQuaternion.length === 4) {
                faceMesh.quaternion.set(fs.faceQuaternion[0], fs.faceQuaternion[1], fs.faceQuaternion[2], fs.faceQuaternion[3]);
              }
            }
          } else {
            // Для обычных граней (не Flora): восстанавливаем quaternion из JSON
            if (fs.faceQuaternion && Array.isArray(fs.faceQuaternion) && fs.faceQuaternion.length === 4) {
              faceMesh.quaternion.set(fs.faceQuaternion[0], fs.faceQuaternion[1], fs.faceQuaternion[2], fs.faceQuaternion[3]);
            } else if (fs.faceRotation && Array.isArray(fs.faceRotation)) {
              faceMesh.rotation.set(fs.faceRotation[0], fs.faceRotation[1], fs.faceRotation[2]);
