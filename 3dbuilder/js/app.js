...
function canPlace(center, kind, ignore){
    ...
    // Check collision with Flora bowl protrusions
    // Skip if placing directly above/below (vertical stacking - same X,Z but different Y)
    // Skip if the new block also has Flora faces (Flora-to-Flora can overlap)
    var newBlockHasFlora = false;
    if (customKinds && customKinds[kind] && customKinds[kind].faceTypes) {
      var ft = customKinds[kind].faceTypes;
      newBlockHasFlora = ft.front === 'Flora' || ft.back === 'Flora' || 
                         ft.left === 'Flora' || ft.right === 'Flora';
    }
    
    if (!newBlockHasFlora) {
      for (var fi = 0; fi < objects.length; fi++){
        var fo = objects[fi];
        if (!fo || !fo.userData) continue;
        if (ignore && (fo === ignore || fo.uuid === ignore.uuid)) continue;
        
        // Skip Flora collision only for TRUE vertical stacking
        // (same X,Z AND Y is above/below the block, not at same level)
        var sameXZ = Math.abs(fo.position.x - center.x) < 0.1 && 
                     Math.abs(fo.position.z - center.z) < 0.1;
        var foH = getHalf(fo.userData.kind || 'Void');
        var newH = getHalf(kind);
        var verticalStack = sameXZ && Math.abs(center.y - fo.position.y) > (foH.y + newH.y - 0.1);
        if (verticalStack) continue;
        
        var floraBoxes = getFloraBoxes(fo);
        for (var bi = 0; bi < floraBoxes.length; bi++){
          var fb = floraBoxes[bi];
          var floraEPS = 0.02;
          var floraOverlapX = test.max.x > fb.min.x + floraEPS && test.min.x < fb.max.x - floraEPS;
          var floraOverlapY = test.max.y > fb.min.y + floraEPS && test.min.y < fb.max.y - floraEPS;
          var floraOverlapZ = test.max.z > fb.min.z + floraEPS && test.min.z < fb.max.z - floraEPS;
          
          if (floraOverlapX && floraOverlapY && floraOverlapZ){
            return false;
          }
        }
      }
...
// ===== Custom kinds serialization for JSON export/import =====
  ...
  function restoreCustomKindsFromPayload(map){
    ...
    for(var kind in map){
      ...
      // Try to get geometry from base kind (new format)
      var baseKind = data.baseKind || 'Bion';
      if(baseGeom && baseGeom[baseKind]){
        merged = baseGeom[baseKind];
      }
      if(faceGeoms && faceGeoms[baseKind]){
        for(var i = 0; i < dirs.length; i++){
          var d = dirs[i];
          if(faceGeoms[baseKind][d]){
