...
function restoreScene(snapArr){
  ...
  for(var si=0; si<snapArr.length; si++){
    ...
    if(existingObj){
      ...
      // Восстанавливаем поворот: quaternion (точнее) → rotation (fallback)
      if (s.quaternion && Array.isArray(s.quaternion) && s.quaternion.length === 4) {
        existingObj.quaternion.set(s.quaternion[0], s.quaternion[1], s.quaternion[2], s.quaternion[3]);
      } else {
      ...
      if(s.type === 'solid'){
        ...
        for(var dir in s.faces){
          ...
          if(faceMesh){
            ...
            // Заменяем геометрию если тип грани изменился
            // Важно: сравниваем с ТЕКУЩИМ типом грани, а не с типом куба
            ...
            var needsGeometryReplacement = (desiredType !== currentType) && 
                                           faceGeoms && faceGeoms[desiredType] && faceGeoms[desiredType][dir];
            ...
            // Приоритет: quaternion (точнее) → rotation (fallback) → вычисление для Flora
            if (fs.faceQuaternion && Array.isArray(fs.faceQuaternion) && fs.faceQuaternion.length === 4) {
              faceMesh.quaternion.set(fs.faceQuaternion[0], fs.faceQuaternion[1], fs.faceQuaternion[2], fs.faceQuaternion[3]);
            } else if (fs.faceRotation && Array.isArray(fs.faceRotation)) {
      ...
      if(s.type==='solid'){
        ...
        for(var dir in s.faces){
          ...
          if(faceMesh){
            ...
            // Заменяем геометрию если тип грани отличается от текущего
            // Важно: сравниваем с типом грани созданного куба, а не с типом куба
            var currentFaceType = grp.userData.faceTypes && grp.userData.faceTypes[dir];
            var isZen2Like = (s.kind === 'Zen/2') || (customKinds && customKinds[s.kind] && customKinds[s.kind].zen2Like);
            var isFloraOnZen2 = isZen2Like && desiredType === 'Flora';
            var needsGeometryReplacement = (desiredType !== currentFaceType) && 
                                           faceGeoms && faceGeoms[desiredType] && faceGeoms[desiredType][dir];
    ...
    if (s.type === 'solid'){
      ...
      if (s.faces && grp.userData && grp.userData.faces){
        for (var dir in s.faces){
          ...
          // Сравниваем желаемый тип грани с ТЕКУЩИМ типом грани созданного куба
          var currentFaceType = grp.userData.faceTypes && grp.userData.faceTypes[dir];
          var isZen2Like = (s.kind === 'Zen/2') || (customKinds && customKinds[s.kind] && customKinds[s.kind].zen2Like);
          var isFloraOnZen2 = isZen2Like && desiredType === 'Flora';
          var needsGeometryReplacement = (desiredType !== currentFaceType) && 
